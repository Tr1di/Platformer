...
namespace Tridi
{
    public class SaveFileHandler
    {
        ...
        public SaveGame Load(string profileId, bool allowRestoreFromBackup = true)
        {
            ...
            try
            {
                ...
                loaded = JsonUtility.FromJson<SaveGame>(dataToLoad);
            }
            catch (Exception e)
            {
        ...
        public void Save(SaveGame data, string profileId)
        {
            // base case - if the profileId is null, return right away
            if (profileId == null)
            {
            ...
            // use Path.Combine to account for different OS's having different path separators
            var fullPath = Path.Combine(_dataDirPath, profileId, _dataFileName);
            var backupFilePath = fullPath + BackupExtension;
            try
            {
        ...
        {
            var profileDictionary = new Dictionary<string, SaveGame>();
        ...
        public string GetMostRecentlyUpdatedProfileId()
        {
            ...
            foreach (KeyValuePair<string, SaveGame> pair in profilesGameData)
            {
